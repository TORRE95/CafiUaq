		...
		inicioAjax.onreadystatechange = function(){
			if (inicioAjax.readyState == 4 && inicioAjax.status == 200) {
				...
				if (inicio[0].tipoUsuario=="0") {
					...
					if (inicio[0].tipoUsuario == "2") {
								window.location.assign('repartidor.html');
							}
...
function ordenRepartidor() {
	var id = localStorage.getItem('idUsuario');
	var url = "https://snackapp1.000webhostapp.com/ordenRepartidor.php";
	if (id != "") {
		ordenAjax = new XMLHttpRequest();
		ordenAjax.open("GET", url);
		ordenAjax.send();
		ordenAjax.onreadystatechange = function(){
			if (ordenAjax.readyState == 4 && ordenAjax.status == 200) {
				...
				for (var i = 0; i < orden.length; i++) {
					var info = 
						"<div class='modulos' onclick='confirm("+'"¿Tomar el pedido?"'+")'>"+
							"<div class='orden'>"+
								"<div class='numero'><label>#58"+orden[i].idOrden+"</label></div>"+
								"<div class='hora'><label>"+orden[i].hora+"</label></div>"+
							"</div>"+
...
function panel(){
	...
	if ( document.getElementById("panel").classList.contains('panelActivo') ){
		document.getElementById("panel").classList.add('panelOculto');
		document.getElementById("panel").classList.remove('panelActivo');
	}
	else{
		document.getElementById("panel").classList.remove('panelOculto');
		document.getElementById("panel").classList.add('panelActivo');
	}
